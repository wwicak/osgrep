...
function calculateAverages(results: BenchmarkComparison[]) {
  ...
  for (const result of results) {
    ...
    metrics.tool_call_reduction.push(
      ((without.tool_calls - with_.tool_calls) / without.tool_calls) * 100,
    );
    metrics.grep_elimination.push(without.grep_calls > 0 ? 100 : 0);
    metrics.files_read_reduction.push(
      ((without.files_read - with_.files_read) / without.files_read) * 100,
    );
  }

  return {
    avg_time_improvement:
      metrics.time_improvement.reduce((a, b) => a + b, 0) /
      metrics.time_improvement.length,
    avg_tool_call_reduction:
      metrics.tool_call_reduction.reduce((a, b) => a + b, 0) /
      metrics.tool_call_reduction.length,
    avg_grep_elimination:
      metrics.grep_elimination.reduce((a, b) => a + b, 0) /
      metrics.grep_elimination.length,
    avg_files_read_reduction:
      metrics.files_read_reduction.reduce((a, b) => a + b, 0) /
      metrics.files_read_reduction.length,
...
function generateMarkdownTable(results: BenchmarkComparison[]): string {
  ...
  for (const result of results) {
    const { without_osgrep: without, with_osgrep: with_ } = result;
    const improvement = (
      ((without.tool_calls - with_.tool_calls) / without.tool_calls) *
      100
    ).toFixed(0);
    const shortQuestion =
      result.question.slice(0, 50) + (result.question.length > 50 ? "..." : "");
...
function main() {
  ...
  try {
    ...
    console.log(
      chalk.green(
        `✓ Loaded ${results.length} benchmark result(s) from ${resultsFile}`,
      ),
    );
  } catch (error) {
